...
Original file is located at
    https://colab.research.google.com/drive/14uQXlCiXhW11C2vAriXz_6sXBCYtxd26
...
"""# **Extracting Derived variables and Store them in the file Output Data Structure in drive**"""

# Paths
cleaned_output_folder_path = '/content/drive/MyDrive/test/CleanedOutput/'
...
# Convert "URL ID" columns to float in both dataframes
df['URL_ID'] = df['URL_ID'].astype(float)
excel_df['URL_ID'] = excel_df['URL_ID'].astype(float)

# Load the Excel file into a DataFrame
excel_df = pd.read_excel(Output_Data_Structure , engine='openpyxl')

# Debug print to see first few rows of both DataFrames
print("Your DataFrame:\n", df.head())
print("\nExcel DataFrame:\n", excel_df.head())

# Find columns present in both DataFrames
common_columns = set(df.columns).intersection(set(excel_df.columns))
print("\nCommon Columns:", common_columns)

# Loop through each row in your DataFrame
for index, row in df.iterrows():
    matching_index = excel_df[excel_df['URL_ID'] == row['URL_ID']].index
    if not matching_index.empty:
        for column in common_columns:
            excel_df.at[matching_index[0], column] = row[column]
    else:
        print(f"No match for URL ID {row['URL_ID']} in Excel file.")

# Save the updated Excel DataFrame back to the Excel file for testing purposes
excel_df.to_excel(Output_Data_Structure , index=False, engine='openpyxl')
...
fdf = df1.merge(df2, on='URL_ID', how='outer')\
             .merge(df3, on='URL_ID', how='outer')\
             .merge(df4, on='URL_ID', how='outer')\
             .merge(df6, on='URL_ID', how='outer')\
             .merge(df7, on='URL_ID', how='outer')\
